...
size_t array[ARR_SIZE];
#define ADDR_COUNT 300
...
{
 memset(array,-1,ARR_SIZE*sizeof(size_t));
 ...
 /*for(int i=0;i<found;i++){
  ...
  while(count <32){
    ...
    *faddrs[0];
    delta_1 = rdtsc() - time;
    printf("count: %d \t delta_0: %5zu\t", count,delta_1);
    ...
    *faddrs[0];
    ...
    for(int k=0;k<5;k++){
     for (size_t i = 1; i < 40-2; i++)//(found-2)-1
     {
      *faddrs[i+2];
      *faddrs[i+1];
      *faddrs[i];
