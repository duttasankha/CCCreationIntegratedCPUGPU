...
void populate_rand_addr_buff(datatype *buff,volatile uint64_t** eviction_random,unsigned int num_pages){
  ...
  for(int i =1;i<=num_pages;i++){
    ...
    eviction_random[i] = (uint64_t *)(buff + selected_pages[i-1]*1024);
...
#define EVICT_SET_RATIO 0.99

#define NUM_RNDS 20000
#define NON_CACHED_THRESHOLD 400
#define CACHED_THESHOLD 200
#define EVICTSET_NUM_PAGES 200
...
void evictSetDeterminationFunc(volatile uint64_t** eviction_random,unsigned int num_pages){
  ...
  while(evEndPtr<num_pages){
     ...
     for(int i =0 ; i< NUM_RNDS;i++){
	  ...
	  for(int j =0 ; j<evEndPtr; j++)  
	   flush((void *)eviction_random[j+1]);
	  ...
	  for(int j=0; j<evEndPtr; j++){
	   //for(int k=0; k<5; k++)
	   maccess((void *)eviction_random[j+1]);  
	  }
	  asm volatile ("mfence");
  ...
  fprintf(stderr,"mr: %f \t Num of page addresses to reach eviction: %d\n\n",mr,evEndPtr);
   ...
   for(int i =0 ; i< evEndPtr; i++)  
     flush((void *)eviction_random[i+1]);
   ...
   for(int i=0; i<evEndPtr; i++)
     maccess((void *)eviction_random[i+1]);  
...
//	   if(rnds==0)
//	    printf("TAKEN\n");
	   finalEvictBuff[detectedPages] = eviction_random[currPos];
